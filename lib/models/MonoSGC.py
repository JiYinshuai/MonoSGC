...
from lib.models.layers.coord_conv import CoordConv
...
class MonoSGC(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None):
        ...
        self.roi_h3d = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.roi_h2d_uncer = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.roi_h3d_uncer = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.grd_depth_offset = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.surf_depth_uncer = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        self.att_depth_uncer = nn.Sequential(
            ...
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.grd_depth = nn.Sequential(
            CoordConv(channels[self.first_level], self.head_conv, kernel_size=3, padding=2, dilation=2, bias=False),
            nn.BatchNorm2d(self.head_conv),
            nn.ReLU(inplace=True),
            CoordConv(self.head_conv, self.head_conv, kernel_size=3, padding=2, dilation=2, bias=False),
            nn.BatchNorm2d(self.head_conv),
            nn.ReLU(inplace=True),
            CoordConv(self.head_conv, 2, kernel_size=1, padding=1 // 2, bias=True))
            # 2 for ground depth and ground depth uncertainty

        self.surf_depth_net = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.surf_depth_net_uncer = nn.Sequential(
            nn.Conv2d(channels[self.first_level] + 2 + self.cls_num, self.head_conv, kernel_size=3, padding=1,
                      bias=True),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        self.roi_h2d.apply(weights_init_xavier)
        self.roi_h3d.apply(weights_init_xavier)
        self.roi_h2d_uncer.apply(weights_init_xavier)
        self.roi_h3d_uncer.apply(weights_init_xavier)
        self.grd_depth_offset.apply(weights_init_xavier)
        self.surf_depth_uncer.apply(weights_init_xavier)
        self.att_depth.apply(weights_init_xavier)
        self.att_depth_uncer.apply(weights_init_xavier)
        torch.nn.init.xavier_normal_(self.grd_depth[-1].weight[-1], gain=0.01)
        self.surf_depth_net.apply(weights_init_xavier)
        self.surf_depth_net_uncer.apply(weights_init_xavier)
    ...
    def forward(self, input, coord_ranges,calibs, targets=None, info=None, K=50, mode='train'):
        ...
        '''
        scale_h2d = (coord_ranges[:, 1, 1] - coord_ranges[:, 0, 1]) / (feat.size()[2] * 4)
        scale_depth = 1 / scale_h2d
        grd_depth = self.grd_depth(feat)
        grd_depth[:, 0, :, :] = (-grd_depth[:, 0, :, :]).exp()
        ret['grd_depth'] = grd_depth
        ...
        ret.update(self.get_roi_feat(feat,inds,masks,ret,calibs,coord_ranges,info,cls_ids))
        return ret


    def get_roi_feat_by_mask(self,feat,box2d_maps,inds,mask,calibs,coord_ranges,info,cls_ids):
        ...
        if num_masked_bin!=0:
            ...
            res['batch_idx'] = scale_box2d_masked[:, 0]
            roi_feature_masked = roi_align(feat,scale_box2d_masked,[5, 5])
            coord_ranges_mask2d = coord_ranges[scale_box2d_masked[:,0].long()]

            #map box2d coordinate from feature map size domain to original image size domain，所有2d box在数剧增强之前的原始图像中的坐标，第一维是batch编号
            ...
            info = info['bbox_downsample_ratio'][box2d_masked[:,0].long()].to(device_id)
            ...
            #generate coord maps
            coord_maps = torch.cat([torch.cat([coords_in_camera_coord[:,1:2]+i*(coords_in_camera_coord[:,3:4]-coords_in_camera_coord[:,1:2])/(5-1) for i in range(5)],-1).unsqueeze(1).repeat([1,5,1]).unsqueeze(1),
                                torch.cat([coords_in_camera_coord[:,2:3]+i*(coords_in_camera_coord[:,4:5]-coords_in_camera_coord[:,2:3])/(5-1) for i in range(5)],-1).unsqueeze(2).repeat([1,1,5]).unsqueeze(1)],1)
            ...
            roi_feature_masked = torch.cat([roi_feature_masked,coord_maps,cls_hots.unsqueeze(-1).unsqueeze(-1).repeat([1,1,5,5])],1)

            #compute 3d dimension offset
            size3d_offset = self.size_3d(roi_feature_masked)[:,:,0,0]
            scale_h2d = torch.clamp(box2d_masked[:, 4] - box2d_masked[:, 2], min=1.0) / \
                        torch.clamp((scale_box2d_masked[:, 4] - scale_box2d_masked[:, 2]) * 4, min=1.0)
            scale_depth = 1 / scale_h2d
            roi_h2d = self.roi_h2d(roi_feature_masked).squeeze(1)
            roi_h3d = self.roi_h3d(roi_feature_masked).squeeze(1)
            att_depth = self.att_depth(roi_feature_masked).squeeze(1)
            grd_depth_offset = self.grd_depth_offset(roi_feature_masked).squeeze(1)
            surf_depth_uncer = self.surf_depth_uncer(roi_feature_masked).squeeze(1)
            roi_h2d_uncer = self.roi_h2d_uncer(roi_feature_masked)[:, 0, :, :]
            roi_h3d_uncer = self.roi_h3d_uncer(roi_feature_masked)[:, 0, :, :]
            att_depth_uncer = self.att_depth_uncer(roi_feature_masked)[:, 0, :, :]
            surf_depth_net = self.surf_depth_net(roi_feature_masked).squeeze(1)
            surf_depth_net = (-surf_depth_net).exp()
            surf_depth_net = surf_depth_net * scale_depth.unsqueeze(-1).unsqueeze(-1)
            surf_depth_net_uncer = self.surf_depth_net_uncer(roi_feature_masked)[:, 0, :, :]
            ins_depth_net = surf_depth_net + att_depth
            ins_depth_net_uncer = torch.logsumexp(torch.stack([surf_depth_net_uncer, att_depth_uncer], -1), -1)
            ...
            res['grd_depth_offset'] = grd_depth_offset
            res['surf_depth_uncer'] = surf_depth_uncer
            res['roi_h2d_uncer'] = roi_h2d_uncer
            res['roi_h3d_uncer'] = roi_h3d_uncer
            res['att_depth_uncer'] = att_depth_uncer
            res['offset_3d'] = self.offset_3d(roi_feature_masked)[:, :, 0, 0]
            res['size_3d'] = size3d_offset
            res['scale_box2d_masked'] = scale_box2d_masked
            res['surf_depth_net'] = surf_depth_net
            res['surf_depth_net_uncer'] = surf_depth_net_uncer
            res['ins_depth_net'] = ins_depth_net
            res['ins_depth_net_uncer'] = ins_depth_net_uncer

        else:
            res['batch_idx'] = torch.zeros([1]).to(device_id)
            ...
            res['roi_h2d'] = torch.zeros([1, 5, 5]).to(device_id)
            res['roi_h3d'] = torch.zeros([1, 5, 5]).to(device_id)
            res['att_depth'] = torch.zeros([1, 5, 5]).to(device_id)
            res['grd_depth_offset'] = torch.zeros([1, 5, 5]).to(device_id)
            res['surf_depth_uncer'] = torch.zeros([1, 5, 5]).to(device_id)
            res['roi_h2d_uncer'] = torch.zeros([1, 5, 5]).to(device_id)
            res['roi_h3d_uncer'] = torch.zeros([1, 5, 5]).to(device_id)
            res['att_depth_uncer'] = torch.zeros([1, 5, 5]).to(device_id)
            res['scale_box2d_masked'] = torch.zeros([1, 5]).to(device_id)
            res['surf_depth_net'] = torch.zeros([1, 5, 5]).to(device_id)
            res['surf_depth_net_uncer'] = torch.zeros([1, 5, 5]).to(device_id)
            res['ins_depth_net'] = torch.zeros([1, 5, 5]).to(device_id)
            res['ins_depth_net_uncer'] = torch.zeros([1, 5, 5]).to(device_id)


        return res


    def get_roi_feat(self,feat,inds,mask,ret,calibs,coord_ranges,info,cls_ids):
        BATCH_SIZE,_,HEIGHT,WIDE = feat.size()
        device_id = feat.device
        ...
        res = self.get_roi_feat_by_mask(feat,box2d_maps,inds,mask,calibs,coord_ranges,info,cls_ids)
        return res
