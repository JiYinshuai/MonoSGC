...
def extract_dets_from_outputs(outputs, conf_mode='ada', K=50):
    ...
    roi_h2d = outputs['roi_h2d']
    box2d_cord = outputs['scale_box2d_masked'][:, 1:].cpu().numpy()
    interval_x = (box2d_cord[:, 2] - box2d_cord[:, 0]) / 5
    interval_y = (box2d_cord[:, 3] - box2d_cord[:, 1]) / 5
    # (800, 5)
    roi_cord_x = np.linspace(box2d_cord[:, 0] + interval_x / 2, box2d_cord[:, 2] - interval_x / 2, 5).transpose(1, 0)
    roi_cord_y = np.linspace(box2d_cord[:, 1] + interval_y / 2, box2d_cord[:, 3] - interval_y / 2, 5).transpose(1, 0)
    roi_cord_2d = []
    for i in range(batch * K):
        x, y = np.meshgrid(roi_cord_x[i], roi_cord_y[i])
        roi_cord_2d.append(torch.from_numpy(np.stack([x, y], -1)))

    roi_cord_2d = torch.stack(roi_cord_2d, 0)
    roi_cord_2d = roi_cord_2d.to(roi_h2d.device)
    roi_cord_2d[..., 1] += roi_h2d
    grd_depth_pred = outputs['grd_depth']

    roi_cord_2d = roi_cord_2d.view(batch, -1, 2)
    grd_depth_points_pred = []
    for j in range(batch):
        grd_depth_points_pred_this_batch, _ = soft_get(grd_depth_pred[j], roi_cord_2d[j], True, True)
        grd_depth_points_pred.append(grd_depth_points_pred_this_batch)
    grd_depth_points_pred = torch.stack(grd_depth_points_pred, 0).view(batch, 2, K, 5, 5)

    att_depth = outputs['att_depth'].view(batch, K, 5, 5)
    grd_depth_offset = outputs['grd_depth_offset'].view(batch, K, 5, 5)
    att_depth_uncer = outputs['att_depth_uncer'].view(batch, K, 5, 5)
    surf_depth_uncer = outputs['surf_depth_uncer'].view(batch, K, 5, 5)
    surf_depth = grd_depth_points_pred[:, 0, ...] + (1. / (grd_depth_offset.sigmoid() + 1e-6) - 1.)
    ins_depth_grd = surf_depth + att_depth
    ins_depth_net = outputs['ins_depth_net'].view(batch, K, 5, 5)
    ins_depth_grd_uncer = torch.logsumexp(torch.stack([surf_depth_uncer, att_depth_uncer], -1), -1)
    ins_depth_net_uncer = outputs['ins_depth_net_uncer'].view(batch, K, 5, 5)

    ins_depth_uncer, index = torch.min(torch.stack([ins_depth_grd_uncer, ins_depth_net_uncer], -1), dim=-1)
    ins_depth = torch.gather(torch.stack([ins_depth_grd, ins_depth_net], -1), -1, index.unsqueeze(-1)).squeeze(-1)
    ...
    ######################
    # merge_prob_grd = (-(0.5 * ins_depth_grd_uncer).exp()).exp()
    # merge_prob_net = (-(0.5 * ins_depth_net_uncer).exp()).exp()
    # merge_prob = torch.cat([merge_prob_grd.view(batch, K, -1), merge_prob_net.view(batch, K, -1)], -1)
    #
    # merge_depth = (torch.sum(torch.cat([(ins_depth_grd * merge_prob_grd).view(batch, K, -1),
    #                                     (ins_depth_net * merge_prob_net).view(batch, K, -1)], -1), dim=-1) /
    #                torch.sum(merge_prob, dim=-1)).unsqueeze(2)
    # if conf_mode == 'ada':
    #     merge_conf = (torch.sum(merge_prob.view(batch, K, -1)**2, dim=-1) / \
    #                   torch.sum(merge_prob.view(batch, K, -1), dim=-1)).unsqueeze(2)
    ######################
